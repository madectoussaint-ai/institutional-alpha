...
for s in d["stocks"]:
    try:
        ...
        price=num(c.iloc[-1]); prev=num(c.iloc[-2]) if len(c)>=2 else None
        old=num(c.iloc[-22]) if len(c)>=22 else num(c.iloc[0])
        ...
        s.update({
            ...
            "currency":cur,
            "change_1d_pct":round(ch1,2) if ch1 is not None else None,
            "change_30d_pct":round(ch30,2) if ch30 is not None else None,
            "momentum":mom,"market_timestamp_utc":now,"market_error":None
        })
